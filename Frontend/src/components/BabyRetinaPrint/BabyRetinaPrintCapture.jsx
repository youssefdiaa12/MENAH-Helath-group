...
const BabyRetinaPrintCapture = () => {
  ...
  const detectRetinaPrint = (canvas) => {
    ...
    let retinaDetected = contours.size() > 0;
  ...
  const capturePhoto = () => {
    if (videoRef.current) {
      ...
      if (!detectRetinaPrint(canvas)) {
        alert("No retina print detected! Please try again.");
        return;
      }
  ...
  const handleSave = () => {
    ...
    const fileName = `Retina_Print_Capture_${babyData.babyName}_${babyData.babyMRN}.png`;
...
export default BabyRetinaPrintCapture;