...
import { useAuth } from "../../context/AuthContext"; // If you use token
...
const ParentProfiles = () => {
  ...
  const fetchParents = async (page) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8080/admin/unverifiedParents",
        {
          method: "POST",
          headers: {
      ...
      if (result.Status) {
        ...
        setFilteredParents(result.Data.data);
        setTotalParents(result.Data.total);
      } else {
  ...
  const totalPages = Math.ceil(totalParents / parentsPerPage);

  // Change page
  const paginate = (pageNumber) => {
    setCurrentPage(pageNumber);
    // No need to call fetchParents here, useEffect will handle it
  };
  ...
  const handleApprove = async (username, phone) => {
    ...
    try {
      ...
      const result = await response.json();
      if (result.Status) {
        fetchParents(currentPage); // Refresh list
      } else {
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-2">
        <div>
          <h1 className="text-xl font-normal text-[#595959] mb-2 p-2">
            Total: {totalParents}
          </h1>
        </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <h2 className="text-xl font-semibold mb-2">Parents Profile Approval</h2>
        <p className="text-gray-600 mb-6">
          Review and approve or reject pending parent accounts below.
        </p>

        {/* Loading and Error States */}
        {loading && <div className="text-center py-4">Loading parents...</div>}
                ...
                {filteredParents.length > 0 ? (
                  filteredParents.map((parent, index) => (
                    <tr key={index} className="border-b border-gray-200">
                      ...
                      <td className="py-3 px-4">{parent.username}</td>
                      <td className="py-3 px-4">{parent.mobile}</td>
                      <td className="py-3 px-4">
                        <div className="flex space-x-2">
                          <button
                            onClick={() => handleApprove(parent.username)}
                            className="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded-md text-sm"
                          >
                            Approve
                          </button>
                          <button
                            onClick={() => handleReject(parent.username)}
          ...
          <div className="flex justify-center mt-6">
            <nav className="flex items-center">
              <button
                ...
                className="px-3 py-1 font-bold bg-gray-200 rounded-l-md disabled:opacity-50"
              >
                &lt;
              </button>
              ...
              <button
                ...
                className="px-3 font-bold py-1 bg-gray-200 rounded-r-md disabled:opacity-50"
              >
                &gt;
              </button>
            </nav>
